...
export default class ListScreen extends Component {
  ...
  deleteConfirmation = (rnumber) => {
    Alert.alert(
      'Status', 
      'Are you sure you want to delete this student?',
      [
        {text: 'Cancel', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
        {text: 'OK', onPress: () => removeStudent(rnumber)}
      ],
      { cancelable: false }
      );
  }
  ...
  render() {
    return (
      <Container>
        ...
        <List vertical={true}>
        <StudentList students={this.state.students} onPress={(rnumber) => {Actions.ViewScreen({rnumber: rnumber});}} onLongPress={(rnumber) => {this.deleteConfirmation(rnumber)}} />
        </List>
        <Text>{this.props.rnumber}</Text>
        </Content>
